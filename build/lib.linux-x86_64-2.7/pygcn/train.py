...
# Load data
adj, features, labels, idx_train, idx_val, idx_test, graph = load_data('/home/feng36/data/dblp/','dblp')
...
def data_sampling(features, idx_train, adj, sample_size, neighbor_sample_size):
    ...
    for idx in selction:
        ...
        for jdx in graph[idx]:
            ...
            concatenated_features.append(torch.cat((features[idx], features[jdx]), dim = 1))
            adj_idx.append()
...
def train(epoch, sample_size, neighbor_sample_size):
    ...
    model.train()
    optimizer.zero_grad()
    samepled_features, sampled_adj = data_sampling(features, idx_train, adj, sample_size, neighbor_sample_size)
